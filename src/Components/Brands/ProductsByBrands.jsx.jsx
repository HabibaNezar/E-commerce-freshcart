...
export default function BrandsByCategory() {
    ...
    console.log("brand from URL Params:", brand);
  
    function getProductRelatedProduct(brand) {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          let allRelatedProducts = allProducts.filter((product) => {
            ...
            return product.brand?.name?.toLowerCase() === brand.toLowerCase();
          });
    ...
    useEffect(() => {
      if (brand) {
        getProductRelatedProduct(brand);
      }
    }, [brand]);
